...
function Index({ blogs }) {
  ...
  useEffect(() => {
    setBlogsFilter(tagSelected ? blogs.filter((blog) => blog.frontmatter.tags.includes(tagSelected)) : blogs);
  ...
  function onSubmit(e) {
    ...
    const result = _.filter(blogs, (blog) => {
      if (tagSelected) {
        if (
          blog.frontmatter.tags.some((tag) => tagSelected === tag) &&
          ...
          <div className="mt-3">
            {tags.map((tag) => (
              <button
                ...
                className={`${
                  tagSelected === tag ? "bg-blue-600 text-white" : "text-blue-600"
                } border-2 border-blue-600  duration-300 rounded-xl px-2 h-[30px] font-semibold m-1`}
              >
                {tag}
              </button>
            ))}
        ...
        <form onSubmit={onSubmit} className="flex items-center mt-8">
          <div className="relative w-[350px]">
            ...
            <input
              ...
              type="text"
              id="simple-search"
              className="bg-gray-50 outline-none border border-gray-300 text-gray-900 text-sm rounded-l-lg focus:ring-blue-500 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500"
              placeholder="Search"
            />
          </div>
        ...
        <article className="py-7  md:p-7 flex flex-wrap gap-5 max-w-full justify-center xl:justify-start">
          {isLoadingTag ? (
            <div className="text-center w-full">
              <PulseLoader color={"#f97316"}  size={25}/>
            </div>
          ) : (
            _.shuffle(blogsFilter, (blog) => new Date(blog.frontmatter.date))
              .reverse()
              .map((blog, index) => <Blog key={index} {...blog} />)
          )}
        </article>
