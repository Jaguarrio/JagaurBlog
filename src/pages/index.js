    ...
    const result = blogs.filter(blog => {
      if (tagSelected) {
        if (blog.frontmatter.tags.some(tag => tagSelected === tag)
          && (blog.frontmatter.title.toLowerCase().includes(search.text.toLowerCase()) ||
            blog.frontmatter.title.toLowerCase().startsWith(search.text.toLowerCase()) ||
            blog.frontmatter.category.toLowerCase().startsWith(search.text.toLowerCase()) ||
            blog.frontmatter.category.toLowerCase().includes(search.text.toLowerCase()))
        ) return blog
      } else {
        if (blog.frontmatter.title.toLowerCase().includes(search.text.toLowerCase()) ||
          blog.frontmatter.title.toLowerCase().startsWith(search.text.toLowerCase()) ||
          blog.frontmatter.category.toLowerCase().startsWith(search.text.toLowerCase()) ||
          blog.frontmatter.category.toLowerCase().includes(search.text.toLowerCase())) return blog
      }
    })
    setBlogsFilter(result)
  }, [search.submit, tagSelected])
      ...
      <header className="pb-8">
        ...
        <div className="flex items-center mt-8">
          <div className="relative w-[350px]">
            ...
            <input value={search.text} onInput={(e) => setSearch({ ...search, text: e.target.value })} type="text" id="simple-search" className="bg-gray-50 outline-none border border-gray-300 text-gray-900 text-sm rounded-l-lg focus:ring-blue-500 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500" placeholder="Search" />
          </div>
          <button onClick={() => setSearch({ ...search, submit: !search.submit })} type="submit" className="p-2.5 text-sm font-medium rounded-r-lg text-white bg-blue-600 border border-blue-600">
